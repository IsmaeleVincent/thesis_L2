...
# -*- coding: utf-8 -*-
"""
Created on Wed May 18 12:07:48 2022
...
mu0=3.5e-3#0.004632543663155012	#+/-	5.46776175965519e-05
...
for k in range(1,2):#len(foldername)):
    ...
    P0= [bcr1, bcr2, mu0,np.pi, 78] # fit_res[0]
    if (fitting):
        B=([3, 0, 2e-3,-2*pi,65],[8, 5, 5e-3,2*pi,100])
    ...
    print(wlpoints)
    def plot_func(x, bcr1, bcr2, mu1, phi,d):
        d=d/np.cos(tilt[k]*rad)
        wl=np.linspace(mu1-3*sigma, mu1+5*sigma, 10000)
        a = rho(wl,lambda_par, mu1, sigma)/sum(rho(wl,lambda_par, mu1, sigma))
        spl = UnivariateSpline(wl, a, k=4, s=0)
